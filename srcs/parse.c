...
static BOOL	AllocatedDataInit(t_data *data, BOOL is_stdin)
{
	data->payload_len = 0;
...
static t_data *DataInit(t_env *env)
{
	if (env->data)
	{
...
static BOOL StringFlag(t_data *data, const char *argv)
{
...
static BOOL error_initialize(t_data *data, const char *message, const char *payload)
{
...
static BOOL read_fd(int fd, const char *filename, t_env *env, BOOL is_stdin)
{
	uint8_t	buffer[BUFF_SIZE];
	int16_t	readed_to_buff;

	t_data	*data = DataInit(env);
...
BOOL parse(int argc, char *argv[], t_env* env)
{
	...
	if (ft_strcmp(argv[i], "md5") == 0) env->hash_func = md5;
	else if (ft_strcmp(argv[i], "sha256") == 0) env->hash_func = sha256;
	else return print_and_exit(ENCODE_ABSENT, argv[i], FALSE);
